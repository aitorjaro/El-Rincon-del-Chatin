...
class RedsysAPI{
	...
	function encrypt_3DES($message, $key) {
        // Se genera un IV aleatorio para cada cifrado
        ...
        // Se devuelve el texto cifrado y el IV
        return array('ciphertext' => $ciphertext, 'iv' => $iv);
    }
	...
	function createMerchantSignature($key) {
        ...
        $order = $this->getOrder();
        $keyResult = $this->encrypt_3DES($order, $key);
        $key = $keyResult['ciphertext'];
        $iv = $keyResult['iv']; // Asegúrate de almacenar este IV para su uso en el descifrado
