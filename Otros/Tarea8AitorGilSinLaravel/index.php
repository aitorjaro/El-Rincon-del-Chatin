...
<section class="principal">
    ...
    if (isset($_GET["action"]) && isset($_GET["id"]) && $_GET["action"] == "get_datos_autor") {
        ...
        $app_info = file_get_contents('http://localhost/Tarea8AitorGilSinLaravel/api.php?action=get_datos_autor&id=' . $_GET["id"]);
                ...
                <ul>
                    ...
                    <?php foreach ($app_info->libros as $libros): ?>
                        <li>
                            ...
                            <a
                                href="<?php echo "http://localhost/Tarea8AitorGilSinLaravel/cliente.php?action=get_datos_libro&id=" . $libros->id ?>">
        ...
        <a class="enlaceVolver" href="http://localhost/Tarea8AitorGilSinLaravel/cliente.php?action=get_listado_autores"
        ...
        $app_info = file_get_contents('http://localhost/Tarea8AitorGilSinLaravel/api.php?action=get_datos_libro&id=' . $_GET["id"]);
        // Se decodifica el fichero JSON y se convierte a array
        $app_info = json_decode($app_info);
        ?>
        <section class="englobaDescripcion">
            ...
            <a class="datosAutor" href="http://localhost/Tarea8AitorGilSinLaravel/cliente.php?action=get_datos_autor&id=<?php echo $app_info->idAutor ?>"
            ...
            <section class="centro">
            <a class="enlaceVolver" href="http://localhost/Tarea8AitorGilSinLaravel/cliente.php?action=get_listado_libros" alt="Lista de libros">Volver a
    ...
    } else //sino muestra la lista de autores
    {
        ...
        $lista_autores = file_get_contents('http://localhost/Tarea8AitorGilSinLaravel/api.php?action=get_listado_autores');
                ...
                <section class="autores">
                    ...
                    <?php foreach ($lista_autores as $autores): ?>
                            <li>
                                <!-- Enlazamos cada nombre de autor con su informacion (primer if) -->
                                <a
                                    href="<?php echo "http://localhost/Tarea8AitorGilSinLaravel/cliente.php?action=get_datos_autor&id=" . $autores->id ?>">
